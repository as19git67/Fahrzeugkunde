...
import { calculateSpeedRunResult, SPEED_RUN_WRONG_PENALTY_SECONDS } from "@/lib/scoring";
...
export function GameScreen({ vehicleId, vehicleName, vehicleData, onFinished }: Props) {
  const { state, currentQuestion, elapsedSeconds, startGame, answerQuestion, nextQuestion } = useGame();
  ...
  // Finished – Dauer kommt aus dem Hook (echte Zeit bis zum Ende), der
  // Speed-Run-Score aus der Zeit statt aus den Einzelpunkten.
  useEffect(() => {
    if (state.phase === "finished" && !finishedRef.current) {
      finishedRef.current = true;
      const durationSeconds = state.durationSeconds ?? elapsedSeconds;
      const score =
        mode === "speed_run"
    ...
    return (
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-md">
          <ModeCard
            title="⏱ Time Attack"
            desc={`Möglichst viele Punkte in ${TIME_ATTACK_DURATION} Sekunden – schnelle richtige Antworten zählen mehr`}
          ...
          <ModeCard
            title="🚀 Speed Run"
            desc={`${SPEED_RUN_TARGET} richtige Antworten so schnell wie möglich – jeder Fehler kostet ${SPEED_RUN_WRONG_PENALTY_SECONDS} s`}
  ...
  return (
    <div className="flex flex-col gap-4 w-full max-w-xl mx-auto">
      {/* Header: Punkte bzw. Zeit + Fortschritt */}
      <div className="flex items-center justify-between bg-zinc-900 rounded-2xl px-4 py-3 relative">
        {mode === "time_attack" ? (
          ...
          <>
            {/* Speed-Run wird nach Zeit gewertet – die Uhr ist hier die Hauptanzeige */}
            <div className="flex flex-col">
              <span className="text-xs text-zinc-400 uppercase tracking-wider">Zeit</span>
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <span className="text-xs text-zinc-500">
            {mode === "speed_run" && state.totalAnswers > state.correctAnswers
              ? `${state.totalAnswers - state.correctAnswers} Fehler · +${(state.totalAnswers - state.correctAnswers) * SPEED_RUN_WRONG_PENALTY_SECONDS} s`
              : `${state.totalAnswers} Fragen`}
          </span>
        </div>

        {/* Punkte-Popup nur im Time-Attack – im Speed-Run zählt die Zeit */}
        <ScorePopup
          delta={mode === "time_attack" && state.phase === "answer_feedback" ? state.lastScoreDelta : null}
        />
      </div>
